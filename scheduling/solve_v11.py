"""CP-SAT re-solve against the v11 grid.

INPUTS:
  - student_requests.csv : 847 students x ~8 requested courses each
  - grid_v11.csv         : teacher x period grid with section capacities
...
with open("grid_v11.csv", encoding="utf-8-sig", newline="") as f:
...
print(f"Sections (v11):  {len(sections)}")
...
print()
print("-" * 70)
print("v9 vs v11 comparison")
print("-" * 70)
print(f"  v9 complete schedules:  257 / 847  (30.3%)")
print(f"  v11 (this solve):       {complete} / {N}  ({complete/N:.1%})")
delta = complete - 257
print(f"  Delta:                  {delta:+d} students  ({delta/N*100:+.1f} pp)")
print(f"  v9 placements:          5007 / 5852  (85.6%)")
print(f"  v11 placements:         {total_placed} / {total_requests}  ({total_placed/total_requests:.1%})")
print(f"  Delta:                  {total_placed - 5007:+d} placements")

# ---------- Write outputs ---------------------------------------------
out_csv = "student_offer_v11.csv"
...
for k, sec in enumerate(sections):
    ...
    v11 = section_counts[k]
    if abs(v11 - v9) >= 5:
        changes.append((sec, v9, v11))
changes.sort(key=lambda t: -(t[2]-t[1]))
for sec, v9, v11 in changes[:20]:
    print(f"  {sec['period']:>3}  v9={v9:>2} -> v11={v11:>2}  ({v11-v9:+d})  "
